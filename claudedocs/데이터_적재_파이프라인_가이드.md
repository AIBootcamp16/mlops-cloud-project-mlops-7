...
│   └── pm10/
│       └── latest.json               # 매시간 덮어쓰기 (고정 경로)
├── ml_dataset/                       # ML 데이터셋 (Parquet, 34 features)
│   └── latest.parquet                # 매시간 덮어쓰기 (고정 경로)
└── weather_pm10_integrated_full.csv  # 마스터 데이터셋 (학습용, 21개월 Rolling Window)
```

**📌 고정 경로 방식 장점**:
- ✅ 월/연도 변경 걱정 없음 (항상 같은 경로)
- ✅ API 단순화 (`ml_dataset/latest.parquet` 고정 사용)
- ✅ 스토리지 최소화 (파일 5개만 유지)
- ✅ 파일 관리 불필요 (자동 덮어쓰기)
...
ml_key = "ml_dataset/latest.parquet"
```
...
class WeatherDataS3Handler:
    ...
    def save_ml_dataset()                         # ML 데이터셋 저장 (고정 경로 덮어쓰기)
    def load_latest_ml_dataset()                  # 최신 ML 데이터셋 로드 (고정 경로)
