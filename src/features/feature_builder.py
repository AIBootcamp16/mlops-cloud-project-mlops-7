...
def add_engineered_features(df: pd.DataFrame, include_labels: bool = False) -> pd.DataFrame:
    ...
    # 출퇴근 시간 여부
    df['is_rush_hour'] = df['hour'].isin([7, 8, 9, 18, 19, 20]).astype(int)
    ...
    if 'temperature' in df.columns:
        ...
        df['heating_needed'] = (df['temperature'] < 10).astype(int)

        df['cooling_needed'] = (df['temperature'] > 25).astype(int)
    ...
    df['is_metro_area'] = df['station_id'].isin(metro_stations).astype(int)
    df['is_coastal'] = df['station_id'].isin(coastal_stations).astype(int)
    ...
    if 'pm10' in df.columns:
        ...
        df['pm10_grade'] = pd.cut(
            ...
            labels=['good', 'moderate', 'bad', 'very_bad']
        )

        # 마스크 필요 여부
        df["mask_needed"] = (df['pm10'] > 50).astype(int)

        # 3️⃣ 야외활동 가능 여부 (pm10 <= 80) → int
        df["outdoor_activity_ok"] = (df['pm10'] <= 80).astype(int)
